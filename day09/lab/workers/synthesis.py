...
Hãy trả lời câu hỏi dựa vào tài liệu trên."""
    ...
    if not answer:
        if not chunks:
            answer = "Không đủ thông tin trong tài liệu nội bộ để trả lời."

        else:
            lines = []

            if policy_result.get("exceptions_found"):
                lines.append("Có ngoại lệ policy cần lưu ý:")
                for ex in policy_result["exceptions_found"]:
                    lines.append(f"- {ex.get('rule', '')} [{ex.get('source', 'policy_refund_v4.txt')}]")

            lines.append("Thông tin tìm được từ tài liệu nội bộ:")

            for chunk in chunks:
                source = chunk.get("source", "unknown")
                text = chunk.get("text", "").strip().replace("\n", " ")
                lines.append(f"- {text[:250]}... [{source}]")
